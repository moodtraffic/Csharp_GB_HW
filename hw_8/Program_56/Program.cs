 ...
 * Затем находит строку с наименьшей суммой
 */
...
for (int i = 0; i < M; i++) {
    rowSum[i] = arrayGetSum(matrixGetRow(matrix, i));

    if (rowNum == -1 || rowSum[i] < rowSum[rowNum]) {
        rowNum = i;
    }
}

Console.WriteLine($"A row #{rowNum+1} has min summ = {rowSum[rowNum]}");
Console.WriteLine();
