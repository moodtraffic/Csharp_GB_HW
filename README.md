...
## Восьмой семинар / hw_8

## Девятый семинар / hw_9

- **задача 62** – программа спирально заполняет массив 4x4 (массив заполняется по часовой стрелке от периферии к центру)

> Решил сделать общее решение, чтобы подобным способом можно было бы заполниь любую матрицы. В том числе для матриц из одной
> колонки или одной строки. Возможно алгоритм не оптимальный и программа кажется достаточно длинной, но все же
> основные вычисления занимают несколько строк.
> Так же мне показалось, что здесь достаточно легко исппользовать рекурсию - заполнение, например, первого "слоя"
> в матрице 3x3, это тоже самое что заполнение 2го "слоя" в матрице 4x4. Реализовал класс Matrix, метод fill() заполняет
> отдельный "слой". Метод move() вычисляет смещение до следующей ячейки в рамках заданной зоны MatrixArea.
> Технически заполнять матрицу можно начинать с любого угла, но столкнулся с такой проблемой -
> если начать заполнять с последней строки (от левого нижнего у углла или из середины строки),
> то алгоритм пытается идти против часовой стрелки - над данной проблемой еще буду работать.
> В данный момент заполнение всегда начинается с левого верхнего угла.
>
